...
# Configure raw and output data paths through the calling experiment.
...
if __name__ == '__main__':
    ...
    case_dir = os.environ.get("RCAEVAL_EXAMPLE_CASE")
    if not case_dir:
        raise RuntimeError("Set RCAEVAL_EXAMPLE_CASE to a prepared case directory.")
    metric_path = os.path.join(case_dir, "simple_metrics.csv")
    log_path = os.path.join(case_dir, "logts.csv")
    trace_lat_path = os.path.join(case_dir, "tracets_lat.csv")
    trace_err_path = os.path.join(case_dir, "tracets_err.csv")

...
def tensor_to_serializable(tensor):
    """Convert a tensor to a serializable value."""
    if tensor is None:
        return None
    return tensor.cpu().numpy().tolist()


def buffer_item_to_dict(buffer_item: BufferItem) -> dict:
    """Convert a buffer item while handling boundary cases."""
    try:
        result = {}
        for field, value in buffer_item.__dict__.items():
            if isinstance(value, torch.Tensor):
                result[field] = tensor_to_serializable(value)
            elif isinstance(value, dict):
                # Convert nested dictionaries.
                result[field] = {k: tensor_to_serializable(v) if isinstance(v, torch.Tensor) else v
...
class NaiveReplayBuffer(ABC):
    ...
    def append(self, experience: Experience) -> None:
        ...
        # for item in items:
        #     print(f"len(action_mask)8 = {item.action_mask.size()}")
        # if not self.packing_samples:
        #     items = remove_padding_in_sequences(items)
        # for item in items:
        #     print(f"len(action_mask)9 = {item.action_mask.size()}")
        ...
        for item in self.items:
            print(f"len(action_mask)10 = {item.action_mask.size()}")
    ...
    def to_data_list(self):
        ...
        for idx, item in enumerate(self.items):
            converted = buffer_item_to_dict(item)

...
class DeepspeedStrategy(ABC):
    ...
    def save_rollout_data(self, data, save_dir, episode, steps):
        ...
        # Each process saves its own data.
        rank = self.get_rank()
        process_file = os.path.join(save_dir, f'episode_{episode}_step_{steps}_rank_{rank}.json')
        ...
        # Rank zero merges the process-local data.
        if rank == 0:
            all_data = []
            # Collect data from all processes.
            for r in range(self.world_size):
                rank_file = os.path.join(save_dir, f'episode_{episode}_step_{steps}_rank_{r}.json')
                with open(rank_file, 'r', encoding='utf-8') as f:
                    rank_data = json.load(f)
                    all_data.extend(rank_data)
            # Save the merged data.
            merged_file = os.path.join(save_dir, f'episode_{episode}_step_{steps}.json')

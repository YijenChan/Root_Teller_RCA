...
def reward_func(queries, prompts, labels, save_path):
    ...
    with open(ground_truth_path, "r") as f:
        ...
        try:
            ...
            if len(results) == 0:
                ...
                if label in results:
                    ...
                    if label_index < 9:
                        content_score = 0.1 * (9 - label_index)
                    else:
                        content_score = 0.1
                hull_penalty = 0
                for result in results:
                    if result not in all_possibilities:
                        hull_penalty += 0.01
                hull_penalty += 0.01 * (len(results) - len(set(results)))
        ...
        with open(save_path, "a+") as f:
            ...
            f.write("format_score=" + str(format_score) + "\n")
            f.write("content_score=" + str(content_score) + "\n")
            f.write("hull_penalty=" + str(-hull_penalty) + "\n")
            f.write("score=" + str(format_score + content_score - hull_penalty) + "\n")
            f.write(str(response) + "\n\n")
            f.write("=" * 30 + "\n")
        return torch.tensor([format_score + content_score - hull_penalty], dtype=torch.float32)

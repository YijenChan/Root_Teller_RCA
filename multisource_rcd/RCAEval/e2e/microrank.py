...
def get_pagerank_graph(df):
    ...
    :return
        operation_operation stores child nodes in the call graph.
        operation_operation[operation_name] = [operation_name1 , operation_name1 ] 

        operation_trace stores the operations visited by each trace (upper-right coverage graph).
        operation_trace[traceid] = [operation_name1 , operation_name2]

        trace_operation stores the traces that visit each operation (lower-left coverage graph).
        trace_operation[operation_name] = [traceid1, traceid2]  
        
        pr_trace stores the operations visited by each trace ID without deduplication.
